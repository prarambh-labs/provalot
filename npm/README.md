...
`npm/install.js` downloads the release binary for your platform and refuses to install it unless its SHA-256 matches the digest recorded in `npm/checksums.json` (filled from the release's `*.sha256` files by `scripts/npm-checksums.sh vX.Y.Z`; the release workflow does this before `npm publish`). `PROVALOT_BINARY_URL` overrides the download URL only when paired with `PROVALOT_BINARY_SHA256`.
...
Bump the version in `Cargo.toml`, `npm/package.json` and `.claude-plugin/plugin.json`, commit, then:

```sh
git tag vX.Y.Z && git push origin vX.Y.Z
```

The `release` workflow checks the three versions match the tag, builds binaries for five targets, attaches them to a GitHub release, then publishes to crates.io and npm with OIDC trusted publishing (npm packages carry provenance). Never move a tag after it has built: the npm installer pins that tag's checksums, so cut a new version instead.
